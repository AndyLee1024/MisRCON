...
class TasksWidget extends Component {
  ...
  addTask = () => {
    this.props.dispatch(
      TasksActions.addTaskAndScheduleCron({
        ...this.state.task,
        id: Date.now(),
        serverId: getActiveServer(this.props.servers).id
      })
    );
  };
  ...
  render() {
    const activeServerTasks = this.props.tasks.filter(
      task => task.serverId === getActiveServer(this.props.servers).id
    );
    const fuzzyList = fuzzy
      .filter(this.state.filterValue, activeServerTasks, {
        extract: task => task.name
      })
      .map(task => task.string);
    const filterList = activeServerTasks.filter(
