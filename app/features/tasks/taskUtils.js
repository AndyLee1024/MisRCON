...
export function scheduleDateTimeTask(task, dispatch) {
  return cron.scheduleJob(
    replaceTimeOfDate(task.time, task.date).toDate(),
    () => {
      misrcon
        .sendRCONCommandToServer(task)
        .then(res => {
...
export const addTaskToLocalStorage = newTask => {
  store.set(
    'savedTasks',
    [].concat(
      store.get('savedTasks').filter(task => task.name !== newTask.name),
      newTask
    )
  );
};
...
export const removeTaskFromLocalStorageByName = name => {
  store.set(
    'savedTasks',
    store.get('savedTasks').filter(task => task.name !== name)
  );
};

export const incRunsByOne = (i, name) =>
  i.name === name ? { ...i, runs: i.runs + 1 } : i;

// increase the run amount by one in local storage
export function incrementTaskInLocalStorageByName(name) {
  store.set(
    'savedTasks',
    store.get('savedTasks').map(task => incRunsByOne(task, name))
  );
}
